...
const Prism = ({
  ...
  useEffect(() => {
    ...
    const fragment = `
      ...
      uniform float uGlow; uniform vec2 uOffsetPx; uniform float uNoise;
      uniform float uSaturation; uniform float uScale; uniform float uHueShift;
      uniform float uColorFreq; uniform float uBloom; uniform float uCenterShift;
      uniform float uInvBaseHalf; uniform float uInvHeight; uniform float uMinAxis;
      ...
      void main(){
        ...
        mat2 wob=mat2(1.0);
        if(uUseBaseWobble==1){ float t=iTime*uTimeScale; float c0=cos(t+0.0),c1=cos(t+33.0),c2=cos(t+11.0); wob=mat2(c0,c1,c2,c0); }
        for(int i=0;i<100;i++){
    ...
    const render = (t) => {
      ...
      if (animationType === 'hover') {
        ...
        rotBuf.set([1,0,0,0,1,0,0,0,1]); program.uniforms.uRot.value = rotBuf;
      }
    ...
    if (suspendWhenOffscreen) {
      ...
      io.observe(container); startRAF(); container.__prismIO = io;
    } else {
      startRAF();
    }
